...
- Note par jour : champ note optionnel dans la modale d'un jour (destination, avec qui, ressenti). Un petit point blanc marque les jours annotés dans la grille ; une note peut exister sans catégorie (jour non « moto »), et vider note + catégorie supprime l'entrée. Modèle de données passé de `"<catId>"` à `{ cat, note, km }` avec migration automatique des anciennes données.
